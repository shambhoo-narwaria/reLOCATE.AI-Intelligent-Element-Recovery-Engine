...
3. **reLOCATE.AI Recovers the Element in 2 Stages**:
   - **Stage 1 (Fingerprint Recovery)**: Uses local math (11 scoring rules) and candidate AI reasoning to find the element.
   - **Stage 2 (Accessibility Recovery)**: If Stage 1 fails, uses lightweight ARIA accessibility snapshots (<500 tokens) to find the element.
...
- **Fast & Private**: Uses local math first (0 token cost) and supports self-hosted LLMs (vLLM / Qwen) so sensitive test data stays private inside your infrastructure.
