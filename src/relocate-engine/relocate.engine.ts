...
function loadConfig() {
  try {
    ...
    console.error('[RelocateEngine] Failed to read config.json, using defaults.', err);
  }
...
export class RelocateEngine {
  ...
  async heal(
    ...
    if (needsAI) {
      resolved = await this.runAIReasoning(original, pool, prunedPool, bestMatch.score, config, onPhaseChange);
      triggeredAI = true;
    } else {
      const msg = `[RelocateEngine] Heuristics highly confident (best score: ${bestMatch.score.toFixed(1)}, threshold: 90). Bypassing AI model...`;
      console.log(msg);
      logger.debug(msg);
    }
  ...
  private filterByTagName(original: OriginalElement, candidates: Candidate[]): Candidate[] {
    ...
    if (origTagFiltered || shadowHostTags.length > 0) {
      ...
      if (filtered.length > 0) {
        console.log(`\n[RelocateEngine] ── FILTER 2a: Tag = "${origTagFiltered || 'SLOT (Ignored)'}" (Shadow hosts: ${shadowHostTags.join(', ') || 'none'}) ──`);
        console.log(`[RelocateEngine]    ${filtered.length} of ${candidates.length} candidates survived.`);
        return filtered;
      }

      console.warn(`[RelocateEngine] No candidates match tag "${origTagFiltered || 'SLOT'}" or shadow hosts [${shadowHostTags.join(', ')}]. Falling back to full pool.`);
    }
  ...
  private filterByInputType(original: OriginalElement, candidates: Candidate[]): Candidate[] {
    ...
    if (origTag === 'INPUT' && original.inputType) {
      ...
      if (inputTypeFiltered.length > 0) {
        console.log(`\n[RelocateEngine] ── FILTER 2b: inputType = "${origInputType}" ──────────────────`);
        console.log(`[RelocateEngine]    ${inputTypeFiltered.length} candidates survived.`);
        return inputTypeFiltered;
      }
      console.warn(`[RelocateEngine] No candidates match inputType "${origInputType}". Keeping tag-filtered pool.`);
    }
    return candidates;
  }
  ...
  private rankCandidates(
    ...
    if (scoredPool.length === 0) {
      throw new Error('[RelocateEngine] Scored candidate pool is empty.');
    }

    const bestMatch = scoredPool[0];
    const runnerUp = scoredPool[1];

    console.log(`\n[RelocateEngine] ── STEP 3: Rule-based Scores (Top 3) ───────────────────────────────`);
    scoredPool.slice(0, 3).forEach((r, i) =>
      console.log(`[RelocateEngine]    #${i + 1}  score=${r.score.toFixed(1).padStart(5)}  [ID ${r.candidate.candidateId}] ${r.candidate.functional.tagName}  css="${r.candidate.functional.cssSelector}"  text="${r.candidate.semantic.accessibleName}"`)
    );
    console.log(`[RelocateEngine]    → Best: [ID ${bestMatch.candidate.candidateId}] score=${bestMatch.score}  RunnerUp: ${runnerUp ? `[ID ${runnerUp.candidate.candidateId}] score=${runnerUp.score}` : 'N/A'}`);
  ...
  private async runAIReasoning(
    ...
    console.log(`[RelocateEngine] Triggering AI Reasoning Layer (Top Score: ${bestMatchScore})`);
    this.stats.totalAISelections++;
    if (onPhaseChange) await onPhaseChange('AI');

    try {
      console.log(`[RelocateEngine] Pruning candidate pool for AI: ${candidates.length} -> ${prunedPool.length} (Max limit: ${config.AI_MAX_CANDIDATES || 10})`);
      console.log(`[RelocateEngine] Sending candidate IDs to AI: ${prunedPool.map(c => c.candidateId).join(', ')}`);
      ...
      if (selectedCandidate) {
        ...
        logger.warn(`[RelocateEngine] AI-selected Candidate [ID ${selectedCandidate.candidateId}] ("${selectedCandidate.semantic.accessibleName || 'unlabeled'}") FAILED pre-action safety validation (Failed gates: ${gateResult.failedGates.join(', ')}). Bypassing AI choice.`);
      }
    } catch (err: any) {
      logger.warn(`[RelocateEngine] Error invoking AI reasoning: ${err.message?.split('\n')[0] || err}. Falling back to rule-based evaluation.`);
    }
  ...
  private tryTopCandidates(
    ...
    for (let i = 0; i < Math.min(3, scoredPool.length); i++) {
      ...
      logger.warn(`[RelocateEngine] Heuristic Candidate #${i + 1} [ID ${match.candidate.candidateId}] ("${match.candidate.semantic.accessibleName || 'unlabeled'}") FAILED pre-action safety validation (Failed gates: ${gateResult.failedGates.join(', ')}).`);
    }

    if (!chosenMatch) {
      logger.warn(`[RelocateEngine] Aborting Healing Process: Top 3 heuristic candidates failed pre-action safety validation.`);
      throw new Error(`[RelocateEngine] Healing aborted: Top 3 candidates failed pre-action safety validation (semantic text / visual shape mismatch).`);
    }

    if (fallbackIndex > 0) {
      logger.warn(`[RelocateEngine] Falling back to Heuristic Candidate #${fallbackIndex + 1} [ID ${chosenMatch.candidateId}] due to validation failures on higher ranked candidates.`);
    }
  ...
  private formatCandidates(pool: Candidate[]): string {
    if (pool.length === 0) return '[RelocateEngine]    (empty pool)';
    return pool
      .map(c => {
        ...
        return `[RelocateEngine]    [ID ${id}]  ${tag}  ${depth}${itype}${role}  css="${css}"  text=${text}`;
      }).join('\n');
  }
